...
import os, time, getopt, random, PXPaths  
...
class _Infos:
    ...
    def __init__( self, startTime, endTime, clients, fileTypes, machines ):
        """
            Data structure to be used to store parameters within parser.
        
        """
        
        self.startTime = startTime # Starting time of the pickle->rrd transfer. 
...
Options:
    ...
    - With -s|--start you can specify the start time of the transfer. 
...
def addOptions( parser ):
    ...
    parser.add_option("-s", "--start", action="store", type="string", dest="start", default=MyDateLib.getIsoFromEpoch( time.time() - dayInSeconds ), help="Decide starting time of the update.")    
...
def getOptionsFromParser( parser, logger = None  ):
    ...
    start     = options.start.replace( '"','' ).replace( "'",'')    
    ...
    try: # Makes sure date is of valid format. 
        ...
        t =  time.strptime( start, '%Y-%m-%d %H:%M:%S' )#will raise exception if format is wrong.
        split = start.split()
        currentTime = "%s %s" %( split[0], split[1] )

    except:    
        print "Error. The startin date format must be YYYY-MM-DD HH:MM:SS" 
    ...
    #round both hours 
    start = MyDateLib.getIsoWithRoundedHours( start )    
    ...
    infos = _Infos( startTime = start, endTime = end, machines = machines, clients = clients, fileTypes = fileTypes )   
...
def createRoundRobinDatabase( dataType, client, machine, startTime ):
    ...
    # 1st  rra : keep last 24 hours for daily graphs. 
    # 2nd  rra : keep last 7 days for weekly graphs. 
    # 3rd  rra : keep last 12 months of data.
    rrdtool.create( databaseName, '--start','%s' %( startTime ), '--step', '60', 'DS:latency:GAUGE:60:U:U', 'RRA:AVERAGE:0:1:1440','RRA:MIN:0:1:1440', 'RRA:MAX:0:1:1440', 'RRA:AVERAGE:0:5:2016','RRA:MIN:0:5:2016','RRA:MAX:0:5:2016', 'RRA:AVERAGE:0:1440:365','RRA:MIN:0:1440:365','RRA:MAX:0:1440:365' )    
...
def getMergedData( client, fileType,  machines, startTime, endTime, logger = None ):
    ...
    if len( machines ) > 1 :    
        statsCollection = pickleMerging.mergePicklesFromDifferentMachines( logger = logger , startTime = startTime, endTime = endTime, client = client, fileType = fileType, machines = machines )                           
...
def updateRoundRobinDatabases(  client, machines, fileType, startTime, endTime, logger = None ):
    """
    ...
    for key in dataPairs.keys():
        ...
        if not os.path.isfile( rrdFileName ):  
            ...
            createRoundRobinDatabase( dataType = key, client = client, machine = combinedMachineName, startTime= MyDateLib.getSecondsSinceEpoch(startTime) - 60 )
...
def transferPickleToRRD( infos, logger = None ):
    ...
    for i in range( len( infos.clients ) ):  
        ...
        if pid == 0 :#if child 
            updateRoundRobinDatabases( infos.clients[i], infos.machines, infos.fileTypes[i], infos.startTime, infos.endTime, logger =logger )                           
